...
# 🛡️ 2.4 애플리케이션 보안 미들웨어 (Verified Implementation)

백엔드(FastAPI)는 다음의 보안 제어 계층을 강제한다:
...
1. **Security Headers (`SecurityHeadersMiddleware`)**:
    ...
    - `Content-Security-Policy` (CSP): 기본적으로 `'self'`만 허용

2. **Audit Logging (`AuditLogMiddleware`)**:
    - 모든 민감한 작업(로그인, 증거 조회, 케이스 삭제, 어드민 기능)을 DB에 자동 기록.
    - `ACCESS_DENIED` (403) 이벤트 발생 시 대상 리소스와 함께 보안 로그 생성.

3. **HTTPS Redirection**:
    - Production(`APP_ENV=prod`) 환경에서 모든 HTTP 요청을 HTTPS로 301 강제 리다이렉션.

4. **Case-Level Partitioning**:
    - `require_case_access` 의존성 주입을 통해 사용자의 소속 케이스 외 데이터 접근을 원천 차단.
